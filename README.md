...
[Diora](https://github.com/paritytech/Diora/tree/master/template) project repository, and can
be used to generate a stand-alone template for use in an independent project via the included
[template generation script](https://github.com/paritytech/Diora/blob/master/docs/node-template-release.md).

A ready-to-use template generated this way is hosted for each Diora release on the
[substrate-developer-hub/Diora-node-template](https://github.com/substrate-developer-hub/Diora-node-template)
repository.
...
```
$ ./target/debug/Diora-template-node --dev
```
...
```
$ ./target/debug/Diora-template-node --dev --manual-seal
```

### Docker Based Development

Optionally, You can build and run the Diora node within Docker directly.  
...
```bash
docker build -t Diora-node-dev .
```

Running (takes 1 min to rebuild binaries):

```bash
docker run -t Diora-node-dev
```
